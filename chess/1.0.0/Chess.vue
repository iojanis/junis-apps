...
<template>
    <div
        class="chess-app h-full flex flex-col p-2 sm:p-4 text-foreground select-none pointer-events-auto"
    >
        <!-- Chessboard -->
            ...
            <div
                ...
                :class="{
                    'opacity-60 pointer-events-none':
                        isGameOver && !chessGame?.in_checkmate(),
                }"
            >
                <div
                    class="chess-board-grid-surface w-full h-full border-2 border-stone-500 shadow-lg rounded overflow-hidden"
                >
                    <template
                        v-for="(row, rowIndex) in boardRepresentation"
                        :key="rowIndex"
                    >
                        <div
                            ...
                            :class="
                                getSquareClass(rowIndex, colIndex, squareData)
                            "
                            @click="
                                handleSquareClick(
                ...
                <div
                    v-if="isGameOver && chessGame?.in_checkmate()"
        ...
        <div
            class="mt-3 p-3 bg-white/50 dark:bg-black/50 rounded-lg shadow text-center min-h-[60px] flex flex-col justify-center"
        >
            <p class="text-lg font-bold" :class="statusClass">
                {{ gameStatusMessage }}
            </p>
            <p
                v-if="
            ...
            <p
                v-if="lastMoveInfo && lastMoveInfo.san"
                class="mt-1 text-xs text-muted-foreground text-center h-4"
            >
...
import {
    ...
    ROOK,
    QUEEN,
} from "@/Components/Chess/chess.js";
...
const initializeGame = () => {
    try {
        ...
        chessGame.value = {
            /* fallback object matching 0.x.x API */
            board: () =>
                Array(8)
                    .fill(null)
...
const updateBoardRepresentation = () => {
    if (chessGame.value && typeof chessGame.value.board === "function") {
        ...
        if (
            ...
            rawBoard.length !== 8 ||
            !rawBoard.every((r) => Array.isArray(r) && r.length === 8)
        ) {
            console.error(
                "CRITICAL: chessGame.value.board() did not return a valid 8x8 2D array!",
                JSON.parse(JSON.stringify(rawBoard)),
            );
            boardRepresentation.value = Array(8)
                .fill(null)
                .map(() => Array(8).fill(null));
            return;
        }
...
const possibleMovesForSelectedPiece = computed(() => {
    ...
    if (
        ...
        !chessGame.value ||
        typeof chessGame.value.moves !== "function"
    ) {
        return [];
    }
    return chessGame.value.moves({
...
const getSquareClass = (rowIndex, colIndex, squareData) => {
    ...
    if (
        lastMoveInfo.value &&
        (squareName === lastMoveInfo.value.from ||
            squareName === lastMoveInfo.value.to)
    ) {
    ...
    if (
        chessGame.value &&
        chessGame.value.in_check() &&
        squareData &&
        squareData.type === KING &&
        squareData.color === chessGame.value.turn()
    ) {
...
const isPossibleMoveMarker = (squareName) => {
    ...
    return possibleMovesForSelectedPiece.value.some(
...
const handleSquareClick = (squareName) => {
    if (
        isGameOver.value ||
        !chessGame.value ||
        typeof chessGame.value.get !== "function" ||
        typeof chessGame.value.move !== "function"
    )
        return;
    ...
    if (selectedSquare.value) {
        const isTargetingOwnPiece =
            pieceOnClickedSquare &&
            pieceOnClickedSquare.color === chessGame.value.turn();
        const isPossibleTarget = possibleMovesForSelectedPiece.value.some(
            (m) => m.to === squareName,
        );
...
const canUndo = computed(() => {
    ...
    return (
        chessGame.value &&
        typeof chessGame.value.history === "function" &&
        chessGame.value.history().length > 0
    );
});
...
const gameStatusMessage = computed(() => {
    // Access gameTurnTrigger to make this computed property reactive to game state changes
    gameTurnTrigger.value;
    if (!chessGame.value || typeof chessGame.value.fen !== "function")
        return "Loading...";
    if (chessGame.value.in_checkmate())
        return `${chessGame.value.turn() === "w" ? "Black" : "White"} wins!`;
...
const statusClass = computed(() => {
    ...
    if (chessGame.value.in_stalemate() || chessGame.value.in_draw())
...
const isGameOver = computed(() => {
    // Access gameTurnTrigger to make this computed property reactive to game state changes
    gameTurnTrigger.value;
    return chessGame.value && typeof chessGame.value.game_over === "function"
        ? chessGame.value.game_over()
        : true;
});
