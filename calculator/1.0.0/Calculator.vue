...
<template>
  <div class="calculator-app h-full flex flex-col p-4 text-foreground">
    ...
    <div class="mb-4 p-4 bg-muted/50 rounded-lg shadow">
      <div class="text-right text-sm text-muted-foreground h-6 truncate" :title="expression">{{ expression || ' ' }}</div>
      <div class="text-right text-3xl font-bold h-10 truncate" :title="display">{{ display }}</div>
    </div>

    <!-- Buttons -->
    <div class="flex-1 grid grid-cols-4 gap-2">
      <!-- Row 1 -->
      <Button @click="clear" variant="destructive" class="col-span-2 h-full text-lg">
        C
      </Button>
      <Button @click="backspace" variant="secondary" class="h-full text-lg">
        ⌫
      </Button>
      <Button @click="appendOperator('/')" variant="outline" class="operator-btn h-full text-lg">
        ÷
      </Button>

      <!-- Row 2 -->
      <Button @click="appendNumber('7')" variant="outline" class="h-full text-xl">7</Button>
      <Button @click="appendNumber('8')" variant="outline" class="h-full text-xl">8</Button>
      <Button @click="appendNumber('9')" variant="outline" class="h-full text-xl">9</Button>
      <Button @click="appendOperator('*')" variant="outline" class="operator-btn h-full text-lg">
        ×
      </Button>

      <!-- Row 3 -->
      <Button @click="appendNumber('4')" variant="outline" class="h-full text-xl">4</Button>
      <Button @click="appendNumber('5')" variant="outline" class="h-full text-xl">5</Button>
      <Button @click="appendNumber('6')" variant="outline" class="h-full text-xl">6</Button>
      <Button @click="appendOperator('-')" variant="outline" class="operator-btn h-full text-lg">
        −
      </Button>

      <!-- Row 4 -->
      <Button @click="appendNumber('1')" variant="outline" class="h-full text-xl">1</Button>
      <Button @click="appendNumber('2')" variant="outline" class="h-full text-xl">2</Button>
      <Button @click="appendNumber('3')" variant="outline" class="h-full text-xl">3</Button>
      <Button @click="appendOperator('+')" variant="outline" class="operator-btn h-full text-lg">
        +
      </Button>

      <!-- Row 5 -->
      <Button @click="appendNumber('0')" variant="outline" class="col-span-2 h-full text-xl">0</Button>
      <Button @click="appendDecimal" variant="outline" class="h-full text-xl">.</Button>
      <Button @click="calculate" variant="default" class="equals-btn h-full text-lg">
        =
      </Button>
    </div>
  </div>
...
const appendNumber = (num) => {
  ...
  if (isNewNumber.value) {
    ...
    if (currentNumber.value === '0' && num !== '.') currentNumber.value = ''; // remove leading zero if not followed by decimal
    currentNumber.value += num
  }
...
const calculate = (isFinalCalculation = true) => {
  if (!operation.value || !previousNumber.value || !currentNumber.value || (isNewNumber.value && isFinalCalculation) ) {
    if (isFinalCalculation && operation.value && previousNumber.value && !currentNumber.value) {
      currentNumber.value = previousNumber.value;
    } else if (isNewNumber.value && isFinalCalculation) {
      return;
    } else if (!operation.value || !previousNumber.value || !currentNumber.value) {
      return;
    }
  }
